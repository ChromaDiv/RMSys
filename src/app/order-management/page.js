...
function OrderManagementContent() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <motion.div
        className="relative px-6 py-6 md:px-10 md:py-8 rounded-3xl bg-white/70 dark:bg-black/50 backdrop-blur-2xl border border-white/20 dark:border-white/10 shadow-xl overflow-hidden flex flex-row items-center justify-between gap-4"
      ...
      >
        ...
        <div className="relative z-10">
          <h1 className="text-2xl md:text-4xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 animate-gradient-x uppercase tracking-wider">{t('orders.title')}</h1>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 px-2">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{t('orders.customerInsights')}</h2>
            ...
            {customerStats.map((customer, idx) => (
              ...
              >
                <div>
                  ...
                  <h3 className="font-bold text-gray-900 dark:text-white text-lg truncate mb-1 text-left" dir="ltr">{customer.name}</h3>
                  <div className="flex items-center gap-2 text-xs text-gray-400 mb-2">
                    <Phone size={10} />
                    <span className="font-medium tracking-wide text-left" dir="ltr">+{customer.phone || 'N/A'}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {customer.patterns.map(p => {
                      // Map pattern string to key
                      const patternKeyMap = {
                        'Late Night': 'lateNight',
                        'Deal Seeker': 'dealSeeker',
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-white/10 flex justify-between items-center mt-auto">
                  <div className="flex flex-col">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">{formatAmount(customer.totalSpent / customer.count)}</span>
                  </div>
                  {customer.phone && (
                    <button
                      onClick={() => {
                        if (isDemo) setShowDemoModal(true);
                        else window.open(`https://wa.me/${customer.phone}`, '_blank');
                      }}
                      className="p-2.5 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-all shadow-md shadow-emerald-500/20 group-hover:scale-110 active:scale-95 flex items-center justify-center"
        ...
        <form onSubmit={handleAddOrder} className="flex flex-col gap-6">
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-1.5">{t('orders.contactNumber')}</label>
                <input
                  className="w-full p-4 rounded-3xl bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-bold"
                  ...
                  .map(item => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-black opacity-70">{formatAmount(item.price)}</span>
                      </button>
                    );
                  })}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-1.5">{t('orders.status')}</label>
                <select
                  className="w-full p-4 rounded-3xl bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:border-indigo-500 outline-none transition-all font-bold"
